...
`0.2.0-s` 在59帧有效点位1数据上复核了同一偏移，留一验证 RMSE
...
默认模型 `0.3.0-s` 增加点位2。按 `2` 切换到点位2后再按 `Tab`，使用56帧
固定偏移 `(+395.59, -1.99, +202.14) mm` 生成点位2，留一 RMSE 为
`6.26 mm`。当前槽为点位1或点位3时按 `Tab`，仍按检测类别自动切换：`远方`
生成点位1，`就地`生成点位3。点位4–9不触发该模型。
