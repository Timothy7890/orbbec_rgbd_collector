...
页面的“点1找点算法版本”可选择版本化的自动找点模型，生成初值后仍可用
`A/D/S/W/Q/E` 微调并按 `C` 保存。`0.1.0` 与 `0.2.0` 使用最高置信度
YOLO 点云的三维中位中心作为参考；其中 `0.2.0` 基于34帧人工确认数据，固定
墙面偏移为 `(+43.30, +9.00, -15.04) mm`。

默认模型 `0.1.0-s` 改用 YOLO Mask 内拟合矩形的粉色几何中心，只允许用于
已经保存墙面坐标系的帧。它基于33帧有效标注（排除人工确认不准确的
`000221`–`000224`），固定墙面偏移为
`(+47.94, +5.86, -19.53) mm`；训练内三维 RMSE 为 `2.50 mm`，留一验证
RMSE 为 `2.58 mm`。同33帧使用 YOLO 语义中心的留一 RMSE 为 `3.25 mm`。
在已有点位1的帧中运行时，页面会同时显示原始点和预测误差；这些数值仍需要
新增独立标注帧验证。
