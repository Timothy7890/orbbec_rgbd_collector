...
页面的“点1找点算法版本”可选择版本化的自动找点模型。“算法找点1”会把
最高置信度 YOLO 点云的三维中位中心转换到当前帧墙面坐标系，再按模型参数
生成点位 1 初值；随后仍可用 `A/D/S/W/Q/E` 微调并按 `C` 保存。`0.1.0`
是基于会话 `20260819_153209_capture` 中 15 帧标注得到的实验版，固定墙面
偏移为 `(+46.07, +9.03, -14.69) mm`。默认的 `0.2.0` 使用同一会话中34帧
人工调整确认的数据，固定偏移更新为 `(+43.30, +9.00, -15.04) mm`，暂不
加入相机到平面距离、Yaw、Pitch 或总倾角补偿。在已有点位 1 的帧中运行时，页面会
显示预测与已有标注的三轴误差和三维距离；该结果属于训练集内验证，后续应
使用新增标注帧继续验证并发布新版本。
